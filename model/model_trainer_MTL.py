...
from model.cv.generator import GeneratorMNIST, weights_init
from model.nlp.generator import SimpleSequenceGenerator
from model.nlp.discriminator import DiscriminatorForMultiLabelClassification
...
class CVTrainer:
    def __init__(self, model,args=None):
        self.model_g = GeneratorMNIST(args.noise_dimension)
        self.model_d = model
        self.id = 0
        self.args = args
    ...
    def weighted_ensemble_distillation(self, args, device, global_ds_dataset, client_indexes, model_dict, pd_dataset_dict,
        ...
        try:
            ...
            for epoch in range(args.ed_epoch):
                for i, _ in  enumerate(global_ds_dataset):
            ...
            for i, _ in enumerate(test_global):
...
class NLPTrainer:
    def __init__(self, model, config, args=None):
        self.model_g = SimpleSequenceGenerator(args.noise_dimension)
        self.model_d = model
        self.id = 0
        self.args = args
        self.model_g.apply(weights_init)
    ...
    def get_model_params(self):
        return (self.model_d.bert.cpu().state_dict(),self.model_d.classifier.cpu().state_dict(),self.model_g.cpu().state_dict())
    ...
    def set_model_params(self,model_parameters):
        model_shared_parameters,model_c_parameters,model_g_parameters = model_parameters
        self.model_g.load_state_dict(model_g_parameters)
        self.model_d.bert.load_state_dict(model_shared_parameters)
        self.model_d.classifier.load_state_dict(model_c_parameters)
    ...
    def get_pd_log(self,client_indexes , device, ds_data, model_dict, pd_dict,num):
        ...
        with torch.no_grad():
            for idx in client_indexes:
                tmp_model.bert.load_state_dict(model_dict[idx][0])
                tmp_model.classifier.load_state_dict(model_dict[idx][1])
    ...
    def weighted_ensemble_distillation(self, args, device, global_ds_dataset, client_indexes, model_dict, pd_dataset_dict,
        ...
        try:
            ...
            student_acc = []
            model_d = self.model_d
            model_d.bert.load_state_dict(avg_model_params[0])
            model_d.classifier.load_state_dict(avg_model_params[1])
        ...
        try:
            ...
            for epoch in range(args.epochs):
                for batch in train_data:
                    ...
                    gen_states = self.model_g(noise)

                    fake_output = self.model_d(external_states=gen_states, input_mask=batch["attention_mask"])
                ...
                if epoch == args.epochs-1:
                    logging.info(
                    'Client Index = {}\tEpoch: {}\tEpoch_Loss: {:.6f}\tGenerator_Loss: {:.6f}\tDiscriminator_Loss: {:.6f}'.format(
